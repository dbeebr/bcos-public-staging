...
| Personalization and start | The confirmed install generates the project instructions itself (rc.9): they exist right after the install, the preview listed them, the receipt declares them as required outputs, no unresolved token remains, a second run changes nothing, a hand edit is a conflict, removing a required output fails `validate-cell.sh`, and the interactive wizard stops on an installer-personalized Cell. For a Cell that predates the step the wizard is tested as before: it asks for surfaces and, separately, the agents' role; it renders through the Cell's own renderer; the paste block records its render inputs and hashes; activation notes stay thin; the role/title answer is kept; activation is recorded as self-reported only; `start-cell.sh` reports the instruction lifecycle. |
