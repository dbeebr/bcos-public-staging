---
bcos_type: doc
id: SETBRAIN-CELL-README
title: "setbrain Cell — Example Active Cell with an App Package"
status: active
created: 2026-07-05
...
# setbrain Cell — Example Active Cell
...
domain capability is **setbrain** — a domain brain that agents query for genre
knowledge, venue profiles, and set-description guidance. Because setbrain is
active and agent-facing, it lives in `apps/setbrain/`, not in `library/`.
...
apps/
  setbrain/       ← the active domain capability (own context, knowledge, prompts)
```
...
4. Read `apps/setbrain/README.md` — the app package.
5. Follow the work-loop chain above in order.

## Rule of thumb

setbrain is an app package because it is **active**: it has its own context
index, knowledge, prompts, and proof loop, and agents query it while doing
other work. When a pattern inside setbrain proves reusable beyond it, a
