...
bcos_type: doc
kind: playbook
id: DJBRAIN-CELL-PLAYBOOK-UPDATE-KNOWLEDGE
title: "Playbook — update djbrain knowledge"
status: active
created: 2026-07-02
...
surface: playbooks
related:
  - ../apps/djbrain/CONTEXT_INDEX.md
---

# Playbook — Update djbrain Knowledge

*(Fictional example.)*

## Trigger

Any change to files under `apps/djbrain/knowledge/`.

## Steps

1. Create a task in `work/` from the template; name the exact knowledge files.
2. Read `apps/djbrain/CONTEXT_INDEX.md`, then only the target files.
3. Append or edit within existing headings; prompts reference headings, so
   never restructure without checking `apps/djbrain/prompts/`.
